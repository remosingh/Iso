...
for (y = 10; y < 500; y += 25) {
  for (x = 10; x < 500; x += 50) {
    if (rowNumber % 2 === 0) {
      ...
      var path = new Path.Circle({
        center: new Point(x + 25, y),
        radius: 5,
        fillColor: 'Yellow',
        strokeColor: 'Black'
      });
      arrayOfPoints.push([x + 25, y]);
    }
  }
...
function fixAngle(vector) {
  if (vectorPrevious) {
    //  console.log("have previous")
    var currentAngle = vector.angle;
    var angleRatio = currentAngle / 45;
    var roundedRatio = Math.round(angleRatio)
    var newAngle = (roundedRatio * 45) - 45;
    return newAngle;
  } else {
...
function getNearestPoint(point) {
  ...
  for (i = 0; i < arrayOfPoints.length; i = i + 1) {
    ...
    var distance = (point - arrayOfPoints[i]).length;
    //  console.log("the distance is" + distance);
    arrayOfDistance.push(distance);
  }
  for (i = 0; i < arrayOfDistance.length; i = i + 1) {
    if (!currentLowest) {
      currentLowest = arrayOfDistance[i]
      positionNumber = i;
    } else if (arrayOfDistance[i] < currentLowest) {
      currentLowest = arrayOfDistance[i];
      positionNumber = i;
    }
  }
  console.log("the lowest value is" + currentLowest + "and point is" + arrayOfPoints[positionNumber]);
  return arrayOfPoints[positionNumber];
}


function processVector(event, drag) {
  vector = event.point - vectorStart;
  vector.angle = fixAngle(vector);
  if (vectorPrevious) {
...
function onMouseDown(event) {
  ...
  // getNearestPoint(event.point)
  var end = vectorStart + vector;
  var create = false;
  if (event.modifiers.shift && vectorItem) {
    var nearestSnapPoint = new Point(getNearestPoint(event.point));
    if (nearestSnapPoint) {
      vectorStart = nearestSnapPoint;
    } else {
